/*
 * ppe-public — App.jsx (version Supabase finale)
 */
      ...
      <div style={S.card}>
        <div style={{ textAlign:"center", marginBottom:26 }}>
          ...
          <p style={{ color:"#6b7a99", fontSize:12, margin:0, lineHeight:1.6, fontFamily:FONT }}>
            Saisissez le code à 8 caractères figurant sur l'avis déposé sur votre véhicule.
          </p>
        </div>
        ...
        <button onClick={submit} disabled={loading} style={{ ...S.btnPrimary, opacity:loading?0.6:1 }}>
...
function StepPlate({ infraction, onConfirm }) {
  ...
  const submit = () => {
    ...
    if (normalizePlate(plate) === normalizePlate(infraction.plate || "")) {
      setError(""); onConfirm();
    } else {
      setError("Ce numéro de plaque ne correspond pas à cet avis. Vérifiez votre saisie.");
    }
  };
    ...
    <div style={S.page}>
      <div style={{ ...S.card, textAlign:"center" }}>
        ...
        <p style={{ color:"#6b7a99", fontSize:12, lineHeight:1.7, marginBottom:28, fontFamily:FONT }}>
          Pour toute question, utilisez le formulaire de contact ci-dessous en indiquant le code figurant sur votre avis.
        </p>
...
function StepResult({ infraction, onReset }) {
  ...
  // Marquer comme consulté si actif
  ...
  const submitContact = async () => {
    ...
    if (!emailOk) { setFormError("Adresse e-mail invalide."); return; }
    setSending(true);
    ...
    <div style={S.pageTop}>
      <div style={{ width:"100%", maxWidth:500 }}>
        ...
        <div style={{
          ...
          borderRadius:"8px 8px 0 0", padding:"16px 22px",
          ...
          <div>
            ...
            <div style={{ fontWeight:700, fontSize:16, fontFamily:FONT }}>
              {isCancelled ? "Infraction annulée" : "Détails de l'infraction"}
            </div>
          </div>
          <div style={{
            display:"inline-block", padding:"3px 10px", borderRadius:20,
            fontSize:10, fontWeight:700, textTransform:"uppercase", fontFamily:FONT,
            background:isCancelled?"#f3f4f6":"#fff0f2",
            color:isCancelled?"#6b7280":"#c8102e",
        ...
        {isCancelled ? (
          <div style={{ background:"white", border:"1px solid #e0dcd5", borderTop:"none", padding:"20px 22px" }}>
            <p style={{ fontSize:13, color:"#444", lineHeight:1.7, margin:0, fontFamily:FONT }}>
              L'infraction initialement enregistrée pour votre véhicule (<strong>{infraction.plate}</strong>) a été annulée.
              Aucune suite ne sera donnée à ce dossier.
            </p>
          </div>
        ) : (
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                  {photos.map((src,i)=>(
                    <a key={i} href={src} target="_blank" rel="noreferrer">
                      <img src={src} style={{ width:"100%", height:120, objectFit:"cover", borderRadius:6, cursor:"pointer" }} />
                    </a>
                  ))}
                </div>
              </div>
        ...
        {!isCancelled && (
          <div style={{ background:"#faf9f7", border:"1px solid #e0dcd5", borderTop:"none", padding:"12px 22px" }}>
            <p style={{ fontSize:11, color:"#6b7a99", margin:0, lineHeight:1.6, fontFamily:FONT }}>
              Toute récidive fera l'objet d'un suivi renforcé.
            </p>
          </div>
        )}
        ...
        {!sent && (
          <div style={{ background:"#f0ede8", border:"1px solid #e0dcd5", borderTop:"none", borderRadius:"0 0 8px 8px", padding:"14px 22px" }}>
            {!showContact ? (
              <button onClick={()=>setShowContact(true)} style={{ ...S.btnOutline, marginTop:0, background:"white" }}>
                ✉ Contacter
              </button>
            ) : (
              <div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                  <Field label="Nom *">
                    <input style={S.input} value={form.lastName} onChange={e=>{set("lastName",e.target.value);setFormError("");}} placeholder="Dupont" />
                  </Field>
                  <Field label="Prénom *">
                    <input style={S.input} value={form.firstName} onChange={e=>{set("firstName",e.target.value);setFormError("");}} placeholder="Jean" />
                  </Field>
                </div>
                <Field label="E-mail *">
                  <input style={S.input} type="email" value={form.email} onChange={e=>{set("email",e.target.value);setFormError("");}} placeholder="jean@example.com" />
                </Field>
                <Field label="Téléphone *">
                  <input style={S.input} type="tel" value={form.phone} onChange={e=>{set("phone",e.target.value);setFormError("");}} placeholder="079 123 45 67" />
                </Field>
                <Field label="Plaque">
                  <input style={{ ...S.input, background:"#f0ede8", color:"#888", cursor:"not-allowed" }} value={infraction.plate||""} readOnly />
                </Field>
                <Field label="Message *">
                  <textarea style={S.textarea} value={form.message} onChange={e=>{set("message",e.target.value);setFormError("");}} placeholder="Décrivez votre situation…" />
                </Field>
                {formError && <div style={S.error}>{formError}</div>}
                <div style={{ display:"flex", gap:10, marginTop:6 }}>
                  <button onClick={()=>setShowContact(false)} style={{ ...S.btnOutline, marginTop:0, flex:1 }}>Annuler</button>
                  <button onClick={submitContact} disabled={sending} style={{ ...S.btnPrimary, marginTop:0, flex:2, opacity:sending?0.6:1 }}>
        ...
        <button onClick={onReset} style={{ ...S.btnOutline, marginTop:14, background:"white", border:"1.5px solid #d0cbc4" }}>
          ← Consulter un autre avis
        </button>
      </div>
    </div>
  );
